								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" id="txtCustomerPhone" readonly="readonly"
													 value="<?php echo isset($orderData) ? $orderData['header']['phone'] : '' ?>"
									...
									<td style="text-align: center;">
										...
										function buildButton($orderDetail, $inArray)
										{
											$buttonCollection = array(
													array("class" => "btn_add_detail_extra btn btn-info btn-sm", "title" => "Add Extra"),
													array("class" => "btn_detail_done btn btn-success btn-sm", "title" => "Ready To Pickup"),
													array("class" => "btn_delete_order_detail btn btn-danger btn-sm", "title" => "Delete")
											);
											$retval = array();
											$count = 0;
											for ($idx = 0; $idx < sizeof($buttonCollection); $idx++) {
												if ($inArray[$idx] == $idx) {
													$retval[$count] = '<a href="javascript:void(0)" data-id="' . $orderDetail["id"] . '" data-headerid="' . $orderDetail["header_id"] . '" data-serviceid="' . $orderDetail["service_id"] . '" class="' . $buttonCollection[$idx]["class"] . '">' . $buttonCollection[$idx]["title"] . '</a>';
													$count++;
												}
											}
										...
										if ($orderData["header"]["payment_status_id"] != 2) {
											if ($d["work_status"] == "0") {
												echo buildButton($d, array(0,1));
											}
										}

										?>
										<?php if ($orderData["header"]["payment_status_id"] != 2): ?>
											<?php if ($d["work_status"] == 0): ?>

											<?php endif; ?>
											<?php if ($d["work_status"] == 0): ?>
												<a href="javascript:void(0)" class="btn_detail_done btn btn-success btn-sm"
													 data-serviceid="<?php echo $d["service_id"]; ?>"
													 data-headerid="<?php echo $d["header_id"]; ?>"
													 data-id="<?php echo $d["id"]; ?>">Ready to Pickup</a>
											<?php elseif ($d["work_status"] == 1): ?>
												<a href="javascript:void(0)" class="btn_detail_taked btn btn-success btn-sm"
													 data-serviceid="<?php echo $d["service_id"]; ?>"
													 data-headerid="<?php echo $d["header_id"]; ?>"
													 data-id="<?php echo $d["id"]; ?>">Pickup</a>
												<a href="javascript:void(0)" class="btn_cancel_pickup btn btn-success btn-sm"
													 data-serviceid="<?php echo $d["service_id"]; ?>"
													 data-headerid="<?php echo $d["header_id"]; ?>"
													 data-id="<?php echo $d["id"]; ?>">Rollback</a>
											<?php elseif ($d["work_status"] == 2): ?>
												<a href="javascript:void(0)"
													 class="btn_detail_reverttake btn btn-success btn-sm"
													 data-serviceid="<?php echo $d["service_id"]; ?>"
													 data-headerid="<?php echo $d["header_id"]; ?>"
													 data-id="<?php echo $d["id"]; ?>">Already Pick up
													at: <?= formatDate("d M Y", $d["pickup_date"]); ?></a>
											<?php else: ?>

											<?php endif; ?>
											<?php
											$showDeleteDetail = false;
											$showDeleteDetail = true;
//													if (($orderData["header"]["order_status_id"] > 0) && (($orderData["header"]["total_payment"] == 0) && ($orderData["header"]["down_payment"] == 0))) {
//													}

											?>
											<?php if ($showDeleteDetail): ?>
												<a href="javascript:void(0)" data-serviceid="<?php echo $d["service_id"] ?>"
													 class="btn_delete_order_detail btn btn-danger btn-sm"
													 data-headerid="<?php echo $d["header_id"]; ?>"
													 data-id="<?php echo $d["id"]; ?>">Delete</a>
											<?php endif; ?>
										<?php else: ?>
											<?php if ($d["work_status"] == 0): ?>
												<a href="javascript:void(0)" class="btn_detail_done btn btn-success btn-sm"
											...
											<?php elseif ($d["work_status"] == 1): ?>
												...
												<a href="javascript:void(0)"
													 class="btn_detail_unfinish btn btn-danger btn-sm"
													 data-serviceid="<?php echo $d["service_id"]; ?>"
													 data-headerid="<?php echo $d["header_id"]; ?>"
													 data-id="<?php echo $d["id"]; ?>">Un Finished</a>
											<?php elseif ($d["work_status"] == 2): ?>

												<?php if ($orderData["header"]["pickup_date"] != ""): ?>
													Already Pick up at: <?= formatDate("d M Y", $d["pickup_date"]); ?>
												<?php else: ?>
													<a href="javascript:void(0)"
														 class="btn_detail_reverttake btn btn-success btn-sm"
														 data-serviceid="<?php echo $d["service_id"]; ?>"
														 data-headerid="<?php echo $d["header_id"]; ?>"
														 data-id="<?php echo $d["id"]; ?>"><?= formatDate("d M Y", $d["pickup_date"]); ?></a>
												<?php endif; ?>
