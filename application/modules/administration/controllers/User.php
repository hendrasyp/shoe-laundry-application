...
class User extends MX_Controller
{
	...
	function get_data_user()
	{
		...
		if (isset($customSearch) && $customSearch != "") {
			$where = "roleid NOT IN (1,2) AND name LIKE '%" . $customSearch . "%' OR phone LIKE '%" . $customSearch . "%'";
		} else {
		...
		foreach ($list as $field) {
			$btnEdit = array(
				...
				"href" => base_url() . 'administration/user/edit/' . $field->id,
				"title" => "",
				"icon" => "fas fa-edit",
...
//          <a class="btn btn-sm btn-danger delete_user" ' . $this->buildDataAttr($field) . ' href="javascript:void(0)" title="Hapus" ><i class="glyphicon glyphicon-trash"></i> Delete</a>';
			...
			$row[] = renderActionButton($btnEdit)  . ' ' . renderActionButton($btnDelete);
	...
	{
		$css = array();
		$js = array('js/modules/user/index.js');
